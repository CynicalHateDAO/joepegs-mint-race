...
class Web3Client(object):
    ...
    The max gas / gas limit are set to the competition values, but the
    priority gas defaults to 1.
    """
    ...
    def build_base_tx(self) -> TxParams:
        ...
        tx: TxParams = {
            ...
            'maxPriorityFeePerGas': Web3.toWei(self.max_priority_fee_gwei, 'gwei'),
    ...
    def sign_tx(self, tx: TxParams) -> SignedTransaction:
        ...
        return self.w3.eth.account.sign_transaction(tx, self.account.privateKey)
    ...
    def get_balance(self) -> float:
        ...
        return float(Web3.fromWei(self.get_balance_wei(), 'ether'))
    ...
    def send_value(self, to_address: ChecksumAddress, amount: float) -> HexStr:
        """Send avax, in avax."""
        return self.send_value_wei(to_address, Web3.toWei(amount, 'ether'))
